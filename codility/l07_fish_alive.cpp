...
// between them, they will eventually meet each other. Then only one fish
// can stay alive − the larger fish eats the smaller one. More precisely, we
// say that two fish P and Q meet each other when P < Q, B[P] = 1 and B[Q] = 0,
...
// each element of array B is an integer with one of the following values: 0, 1;
...
using namespace std;
////////////////////////////////////////////////////////////////////////////////
// Put all downstream swimming fishes on a stack. Any upstream swimming fish
// has to fight(eat) all fishes on the stack. If there is no fish on the stack,
// the fish survives. If the stack has some downstream fishes at the end,
// they also survive.
// Important to note that fish power is not getting reduced with each eat.
int fish_alive(vector<int> A, vector<int> B)
{
        ...
        if (direction == UPSTREAM)
        {
            while(!order.empty())
                if (order.top() > power)
                    ...
                    order.pop();
            if (order.empty())
                ++survivals;  // Upstream fish counted separately
        }
        else // Downstream: Only fish swimming downstream are pushed
            order.push(power);
    }
    survivals += order.size();
    return survivals;

}
////////////////////////////////////////////////////////////////////////////////
TEST_CASE("Fish left alive", "[codility]")
{
    REQUIRE(fish_alive({4, 3, 2, 1, 5}, {0, 1, 0, 0, 0}) == 2);
}
