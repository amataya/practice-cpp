...
// maximum number of ropes of length greater than or equal to K that
// can be created.
//
...
using namespace std;
////////////////////////////////////////////////////////////////////////////////
int tie_ropes(int K, const vector<int> &A)
{
    if (A.empty())
        return 0;

    int counts = 0, // Number of tied ropes whose length >= K
        length = 0; // Length of current rope, might be tied one
    for(const auto &rope: A)
    {
        if (rope >= K)
        {
            ++counts;
...
////////////////////////////////////////////////////////////////////////////////
TEST_CASE("Tie Ropes", "[codility]")
{
    REQUIRE(tie_ropes(4, {1, 2, 3, 4, 1, 1, 3}) == 3);
}