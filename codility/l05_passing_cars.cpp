...
// where 0 ≤ P < Q < N, is passing when P is traveling to the
...
//
// For above example, function should return 5
//
...
using namespace std;
////////////////////////////////////////////////////////////////////////////////
int passing_car_pairs(vector<int> A)
{
    // For each WEST bound in the list, number of pairs will be incremented by
    // the number of EAST bound cars discovered before that WEST bound car
    const uint64_t EAST = 0, WEST = 1;
    uint64_t pairs = 0;
    uint64_t east_bound = 0;
    for (const auto& direction : A)
    {
        ...
        {
            pairs += east_bound;
            if (pairs > 1'000'000'000) // Use C++11 literal formatting
                return -1;
        }
    }
    return pairs;
...
////////////////////////////////////////////////////////////////////////////////
TEST_CASE("Passing Cars", "[codility]")
{
    REQUIRE(passing_car_pairs({0, 1, 0, 1, 1}) == 5);
}
